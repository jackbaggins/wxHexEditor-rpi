...
bool HexVerifyAndPrepare(wxString &hexval, wxString Value_Name, wxWindow *parent)
{
	...
	int x = hexval.find(' ');
	while (x >= 0)
	{ // using hexval.find(' ')>= 0 generate logic true due compiler error
		hexval.Remove(hexval.find(' '), 1);
		x = hexval.find(' ');
	}

	for (unsigned i = 0; i < hexval.Len(); i++)
		if (!isxdigit(hexval[i]) || hexval == ' ')
		{ // Not hexadecimal!
...
GotoDialog::GotoDialog(wxWindow *_parent, uint64_t &_offset, uint64_t _cursor_offset, uint64_t _filesize, unsigned _BlockSize) : GotoDialogGui(_parent, wxID_ANY)
{
	...
	*/
	...
	m_branch->SetSelection(options & OPT_BRANCH_NORMAL ? 0 : options & OPT_BRANCH_END ? 2
																					  : 1);
...
void GotoDialog::EventHandler(wxCommandEvent &event)
{
	...
	options |= m_branch->GetSelection() == 0 ? OPT_BRANCH_NORMAL : m_branch->GetSelection() == 2 ? OPT_BRANCH_END
																								 : 0;
...
{
	if ((event.GetKeyCode() == WXK_BACK) || (event.GetKeyCode() == WXK_RETURN) || (event.GetKeyCode() == WXK_LEFT) || (event.GetKeyCode() == WXK_RIGHT) || (event.GetKeyCode() == WXK_DELETE) || (event.GetKeyCode() == WXK_TAB) || (event.GetKeyCode() == WXK_END) || (event.GetKeyCode() == WXK_HOME) || (event.GetKeyCode() == WXK_CAPITAL) || (event.GetKeyCode() == WXK_RETURN) || (event.ControlDown() && (event.GetKeyCode() == 1	 // CTRL+A
...
{
	if (m_searchtype->GetSelection() == 1)
	{																																																																																																											 // Just for Hex Mode
		if ((event.GetKeyCode() == WXK_BACK) || (event.GetKeyCode() == WXK_RETURN) || (event.GetKeyCode() == WXK_LEFT) || (event.GetKeyCode() == WXK_RIGHT) || (event.GetKeyCode() == WXK_DELETE) || (event.GetKeyCode() == WXK_TAB) || (event.GetKeyCode() == WXK_END) || (event.GetKeyCode() == WXK_HOME) || (event.GetKeyCode() == WXK_CAPITAL) || (event.GetKeyCode() == WXK_RETURN) || (event.ControlDown() && (event.GetKeyCode() == 1	 // CTRL+A
																																																																																																					 || event.GetKeyCode() == 24 // CTRL+X
																																																																																																					 || event.GetKeyCode() == 3	 // CTRL+C
									   ...
									   wxProgressDialog *progress_gauge, wxString &gauge_msg, uint64_t &totalread)
{
	...
	do
	{
		...
		if (readed_prefetch > 0)
		{ // PreLoad file, +%50 speed on SSD buffers.
...
#ifdef _DEBUG_
		std::cout << std::dec << "FindBinary() BACKWARD " << backward_offset << "-" << current_offset << " \t "
				  << "SearchStep:" << search_step << std::endl;
#endif
		if (readed_prefetch > 0)
		{ // PreLoad file, +%50 speed on SSD buffers.
			readed = readed_prefetch;
			...
			if ((options & SEARCH_FINDALL) || !(options & SEARCH_BACKWARDS))
				for (int i = 0; i <= bfr_size - search_size; i++)
				{
					...
					)
					{
						...
						if (!memcmp(ubuf, search, search_size))
						{ // if match found
							ret_ptr->push_back(i);
							if (options & SEARCH_FINDALL)
								continue;
...
//#ifdef __SSE2__
		...
		{ // Normal Operation
			for (int i = 0; i <= bfr_size - std::max(search_size, REG_SZ); i += REG_SZ)
			{
				...
				if (reg1 || reg2)
				{ // If SIMD return a possible match in this block
#ifdef _DEBUG_FIND_
					char s[REG_SZ];
					__m128i *k = (__m128i *)(s);
					...
					for (int j = 0; j < REG_SZ; j++)
					{
						...
						if (bfij == FirstChL || bfij == FirstChU)
						{
							...
							for (int k = ij + 1; (k < bfr_size) && ((k - ij) < search_size); k++)
								bfr[k] = tolower(bfr[k]);
							if (!memcmp(bfr + ij + 1, search + 1, search_size - 1))
							{ // if match found
...
int ReplaceDialog::OnReplace(bool internal)
{
	...
	{
		if (m_searchtype->GetSelection() == 0)
		{ // text search
			PrepareComboBox(true);
	...
	for (unsigned current_offset = 0; current_offset < select->GetSize(); current_offset += BytePerLine)
	{
		...
		if (chkText->GetValue())
		{
			...
			cb += wxString::From8BitData(&buff[current_offset],
										 (current_offset + BytePerLine > select->GetSize()) ? select->GetSize() - current_offset : BytePerLine);
		}
		cb += wxNewline;
	}
}
...
void CopyAsDialog::Copy(void)
{
	if (select->GetState())
	{
		...
		else if (chcCopyAs->GetSelection() == 2)
		{ // Internet
			if (chcOption->GetSelection() == 0)
			{ // html
			...
			else if (chcOption->GetSelection() == 1)
			{ // HTML with Tags
				cb += wxT("<pre><code style=\"color:#000000;\">");
			...
			else if (chcOption->GetSelection() == 2)
			{ // phpBB Forum
				cb += wxT("[code]");
				PrepareFullText(cb, buff);
		...
		else if (chcCopyAs->GetSelection() == 4)
		{ // ASM
			unsigned HexSize = pow(2, chcOption->GetSelection());
			unsigned count = select->GetSize() / HexSize;
...
void ChecksumDialog::EventHandler(wxCommandEvent &event)
{
	...
	else if (event.GetId() == btnCalculate->GetId())
	{
		...
		if (msg != wxEmptyString)
			msg = _("File: ") + F->GetFileName().GetFullPath() + wxNewline + (chkFile->GetValue() && F->IsChanged() ? _("Notice: Checksum includes non-saved changes.") + wxString(wxNewline) + wxNewline : wxNewline) + msg;
...
PreferencesDialog::PreferencesDialog(wxWindow *parent) : PreferencesDialogGui(parent, wxID_ANY)
{
	...
	wxString TempString;
	bool TempBool;
	if (myConfigBase::Get()->Read(_T("ColourHexForeground"), &TempString))
		clrPickerForeground->SetColour(TempString);
	if (myConfigBase::Get()->Read(_T("ColourHexBackground"), &TempString))
		clrPickerBackground->SetColour(TempString);
	if (myConfigBase::Get()->Read(_T("ColourHexBackgroundZebra"), &TempString))
		clrPickerBackgroundZebra->SetColour(TempString);
	if (myConfigBase::Get()->Read(_T("ColourHexSelectionForeground"), &TempString))
		clrPickerSelectionForeground->SetColour(TempString);
	if (myConfigBase::Get()->Read(_T("ColourHexSelectionBackground"), &TempString))
		clrPickerSelectionBackground->SetColour(TempString);
	if (myConfigBase::Get()->Read(_T("AutoShowTagPanel"), &TempBool))
		chkAutoShowTagPanel->SetValue(TempBool);
	if (myConfigBase::Get()->Read(_T("SingleInstance"), &TempBool))
		chkSingleInstance->SetValue(TempBool);

	if (myConfigBase::Get()->Read(_T("UseCustomHexFormat"), &TempBool))
		chkCustom->SetValue(TempBool);
	if (myConfigBase::Get()->Read(_T("CustomHexFormat"), &TempString))
		comboCustomHexFormat->SetValue(TempString);
	comboCustomHexFormat->Enable(chkCustom->IsChecked());
	if (myConfigBase::Get()->Read(_T("useBytesPerLineLimit"), &TempBool))
		chkBytePerLineLimit->SetValue(TempBool);
	spinBytePerLine->Enable(chkBytePerLineLimit->IsChecked());
	if (myConfigBase::Get()->Read(_T("FakeBlockLines"), &TempBool))
		chkFakeBlockLines->SetValue(TempBool);
	textCtlFakeBlockSize->Enable(chkFakeBlockLines->IsChecked());
	if (myConfigBase::Get()->Read(_T("FakeBlockSize"), &TempString))
		textCtlFakeBlockSize->SetValue(TempString);
	spinBytePerLine->Enable(chkBytePerLineLimit->IsChecked());
